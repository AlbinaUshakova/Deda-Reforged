    ...
    <div className="space-y-4 text-[var(--text-primary)]">
      <div className="flex items-center justify-between">
        <div className="text-sm text-[var(--text-secondary)]">
          {interfaceLanguage === 'en' ? 'Round:' : 'Раунд:'} {round+1}
        </div>
        <div className="text-sm text-[var(--text-secondary)]">
          {interfaceLanguage === 'en' ? 'Time:' : 'Время:'} <span className={pill}>{total.toFixed(1)}s</span> (+{penalty}s)
        </div>
      </div>
      ...
      {finished ? (
        <div className="rounded-2xl border border-[var(--progress-bg)] bg-[var(--surface,rgba(255,255,255,0.9))] p-4">
          <div className="mb-2">
            {interfaceLanguage === 'en' ? 'Round complete! Time:' : 'Раунд завершён! Время:'} <span className={pill}>{total.toFixed(1)}s</span>
          </div>
          <button
          ...
          >
            {interfaceLanguage === 'en' ? 'Next round' : 'Следующий раунд'}
          </button>
        </div>
      ) : (
        <div className="text-xs text-[var(--text-tertiary)]">
          {interfaceLanguage === 'en'
            ? 'Match the word with its translation. Each mistake adds 1 second.'
            : 'Совмести грузинское слово с переводом. За ошибку +1 секунда.'}
        </div>
      )}
    </div>
  );
